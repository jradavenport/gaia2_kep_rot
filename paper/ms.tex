...
MIST isochrones at ages of $10^8$, $10^9$, and $10^10$ yr are shown for comparison (blue, black, and red lines). The predicted evolution of a 0.7 M$_\odot$ star is highlighted (black squares). The main sequence shows significant scatter, as well as a gradient in rotation period with slower rotators preferentially above the main sequence. 
Right: Difference in $M_G$ from the $10^9$ yr MIST isochrone, shown as a function of their rotation periods (point color again indicates rotation period, as in Left panel). Blue squares show an increase in the median $M_G$ offset in bins of increasing rotation period. Error bars shown are the standard deviation of $\Delta M_G$ in each bin.
}
\label{fig:cmd_zoom}
\end{figure*}
